...
def load_vectorstore():
    ...
    st.info("首次執行，正在解析 3 份 PDF 並建立向量資料庫（只需一次）...")
    ...
    st.success(f"✅ 已處理 {len(splits)} 個文件區塊，建立完成！")
    return vectorstore

vectorstore = load_vectorstore()
retriever = vectorstore.as_retriever(search_kwargs={"k": 6})

# LLM（使用 GPT-4o-mini，速度快且便宜）
...
if prompt := st.chat_input("例如：班機延誤多久可以理賠？酒醉受傷是否理賠？海外住院有補助嗎？"):
